...
### v2.0 b2 (2026-08-24)
- Fixed a bug where simply hovering a favorite row with a mouse (desktop, no click or swipe) revealed the delete action behind it — `.fav-row:hover` set `background: rgba(var(--accent-rgb), 0.06)`, a near-transparent color that replaced the row's normally opaque background, letting `.fav-row-delete-action` show through underneath. Switched to `box-shadow: inset 0 0 0 999px rgba(var(--accent-rgb), 0.06)`, which layers the same tint on top of the existing opaque background instead of replacing it.
